<?php

/*
 * Copyright (c) 2024 - present nicholass003
 *        _      _           _                ___   ___ ____
 *       (_)    | |         | |              / _ \ / _ \___ \
 *  _ __  _  ___| |__   ___ | | __ _ ___ ___| | | | | | |__) |
 * | '_ \| |/ __| '_ \ / _ \| |/ _` / __/ __| | | | | | |__ <
 * | | | | | (__| | | | (_) | | (_| \__ \__ \ |_| | |_| |__) |
 * |_| |_|_|\___|_| |_|\___/|_|\__,_|___/___/\___/ \___/____/
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * @author  nicholass003
 * @link    https://github.com/nicholass003/
 *
 *
 */

declare(strict_types=1);

namespace Nicholass003\LittleBrother\Protocol\Translator\Handler;

use Nicholass003\LittleBrother\Protocol\ProtocolVersion;
use Nicholass003\LittleBrother\Protocol\Translator\ManualPacketHandler;
use pmmp\encoding\Byte;
use pmmp\encoding\ByteBufferReader;
use pmmp\encoding\ByteBufferWriter;
use pocketmine\network\mcpe\protocol\InteractPacket;
use pocketmine\network\mcpe\protocol\serializer\CommonTypes;

final class InteractPacketHandler extends ManualPacketHandler{

	public function translateInbound(int $protocol, ByteBufferReader $in) : string{
		$out = new ByteBufferWriter();

		$action = Byte::readUnsigned($in);
		Byte::writeUnsigned($out, $action);

		$runtimeId = CommonTypes::getActorRuntimeId($in);
		CommonTypes::putActorRuntimeId($out, $runtimeId);

		if($protocol <= ProtocolVersion::BE_1_21_120){
			if($action === InteractPacket::ACTION_MOUSEOVER || $action === InteractPacket::ACTION_LEAVE_VEHICLE){
				$position = CommonTypes::getVector3($in);
			}else{
				$position = null;
			}
		}else{
			$position = CommonTypes::readOptional($in, CommonTypes::getVector3(...));
		}

		CommonTypes::writeOptional($out, $position, CommonTypes::putVector3(...));

		return $out->getData();
	}

	public function translateOutbound(int $protocol, ByteBufferReader $in) : string{
		return $this->passthrough($in);
	}
}
